...
d.sub.01.match = subset(d.sub.01, targettype == "match")
...
d.sub.02 = subset(d.sub.01, !(set %in% c("t10")))
d.sub.02$set = factor(d.sub.02$set)

    ######### d.sub.03: remove high error participants

d.sub.02.match = subset(d.sub.02, targettype != "mismatch")
table.subj.correct = xtabs(~correctness + subject, data = d.sub.02.match);table.subj.correct
...
d.sub.03 = subset(d.sub.02, !(subject %in% c(22,3,13,9,10)))    # participants
